...
const RecipeFiltering = ({
  ...
  const [previousCategories, setPreviousCategories] = useState(selectedCategories);

  useEffect(() => {
    if (selectedCategories.length > 0 && selectedCategories !== previousCategories) {
      // Only trigger refined search if categories have changed
      onRefinedSearch(selectedCategories);
      setPreviousCategories(selectedCategories); // Store the new selectedCategories
    }
  }, [selectedCategories, onRefinedSearch, previousCategories]);
                      ...
                      <label className="subcategoryLabel">
                        ...
                        <input
                          ...
                          checked={
                            ...
                            categoryLists[parentCategory].subcategories[
                              subcategory
                            ].every((item) => selectedCategories.includes(item))
                          }
