...
const MealPlanPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchRecentlyAddedRecipes = async () => {
            try {
                ...
                const response = await fetch('http://localhost:8000/user/recently-added-recipes/', {
                    method: "GET",
                    headers: {
        ...
        const fetchMealPlans = async () => {
            try {
                const token = localStorage.getItem('token');
                const response = await fetch('http://localhost:8000/meal-plan/view-meal-plans/', {
                    method: "GET",
                    headers: {
                        'Authorization': `Token ${token}`,
                    },
    ...
    // Handle adding recipes to the new meal plan
    const handleRecipeSelection = (recipeId) => {
        setNewMealPlan((prevState) => {
            const selectedRecipes = prevState.selectedRecipes.includes(recipeId)
                ? prevState.selectedRecipes.filter(id => id !== recipeId)
                : [...prevState.selectedRecipes, recipeId];
            return { ...prevState, selectedRecipes };
        });
    };
            ...
            return (
                <div key={mealType}>
                    ...
                    {meal ? (
                        <>
                            {meal.recipe_name}
                            <button onClick={() => alert(`Edit ${mealType} for ${day}`)}>Edit</button>
                        </>
                    ) : (
                        <>
    ...
    const handleAddRecipe = async (mealPlanId, recipeId, day, mealType) => {
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`http://localhost:8000/meal-plan/add-recipe/${mealPlanId}/`, {
                method: 'POST',
                headers: {
                    'Authorization': `Token ${token}`,
                    'Content-Type': 'application/json',
